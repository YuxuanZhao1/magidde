...
allresult =
  ...
  ) %dopar%
  {
    ...
    y = out[, c(1, 3)]
    yIn = setDiscretization(y, level = nlevel)
    lvmodel = list(
      name = "lvD",
...
stopCluster(cl)
save.image(paste0("test_discretization_magi_by", nlevel, ".RData"))

load(file = paste0("test_discretization_magi_by", nlevel, ".RData"))
