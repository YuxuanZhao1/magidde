...
model = stan_model("conditional_expectation.stan")
allresult =
  foreach (
    i = 1:nsim,
    .errorhandling = "pass",
...
stopCluster(cl)
save.image(paste0("conditional_level", nlevel, ".RData"))
load(paste0("conditional_level", nlevel, ".RData"))



compute_infer_rmse = function(stan_fit, tvec_obs){
  traj = rep(NA, tlen_obs)
  
  for(i in 1: tlen_obs){
    re = summary(stan_fit, pars = paste0('x_t[',2^(nlevel) * i - (2^(nlevel)-1),']'))
    traj[i] = re$summary[1]
  }
...
df_generator = function(result_file){
  ...
  for(i in 1 : nsim){
    ...
    summarized = summary(stan_fit, pars = c('r', 'k','tau', 'x_t[1]'))$summary
    ...
    df_rmse_traj$infer_rmse[i] = compute_infer_rmse(stan_fit, tvec_obs = tvec_obs)
